...
function main() {
    ...
    // TODO
    // TODO
    // // NOTE #1 Konversi ke CSG
    const cubeCSG = meshToCSG(cubeMesh);
    const ellCSG = meshToCSG(ellipsoidMesh);

    // // NOTE #2 Operasi boolean: cube dipotong sphere (bisa lanjut dengan objek lain)
    // // NOTE #3 Konversi kembali ke mesh
    const holeOnCubeMesh = new CSGBuilder(cubeCSG)
        .subtract(ellCSG)
        .toMesh();

    // // NOTE #4 Recenter kalau mau (biar ke tengah 0,0,0)
    recenterMesh(holeOnCubeMesh)

    // // NOTE #5 Buffer mesh hasil mesh ke GPU
    ...
    function animate(time) {
        ...
        leftShoulder.setLocalSpec({ translate: [-0.7, 0.45, 0], rotate: [{ axis: "x", angle: armSwing }] });
        rightShoulder.setLocalSpec({ translate: [0.7, 0.45, 0], rotate: [{ axis: "x", angle: -armSwing }] });
